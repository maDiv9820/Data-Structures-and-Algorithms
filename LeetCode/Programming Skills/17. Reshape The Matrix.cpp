...
into a new one with a different size r x c keeping its original data. You are given an 
m x n matrix mat and two integers r and c representing the number of rows and the number 
of columns of the wanted reshaped matrix. The reshaped matrix should be filled with all 
the elements of the original matrix in the same row-traversing order as they were. If the 
...
int main() {
    ...
    for(int row = 0; row < r; row++)
        for(int col = 0; col < c; col++)
            cin >> mat[row][col];
    cin >> r >> c;
    vector<vector<int>> newMat = matrixReshape(mat,r,c);
    for(const vector<int>& v: newMat) {
        for(const int& x: v)
            cout << x << " ";
        cout << endl;
    }
