...
at pivot index 5 and become [4,5,6,6,7,0,1,2,4,4]. Given the array nums after the 
rotation and an integer target, return true if target is in nums, or false if it 
...
'''
class Solution:
    def __bSearch(self, nums: list[int], start: int, end: int, target: int) -> bool:
        while start <= end:
            mid = (start+end)//2
            if nums[mid] == target:
                return True
    ...
    def search(self, nums: list[int], target: int) -> bool:
        ...
        if pivot == -1:
            return False
        return self.__bSearch(nums,pivot+1,len(nums)-1,target)
        
nums = [int(x) for x in input().split()]
target = int(input())
sol = Solution()
print("true") if sol.search(nums,target) else print("false")